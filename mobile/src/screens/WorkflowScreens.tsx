...
export function WorkflowScreen({navigation}: any) {
  ...
  const [showForm, setShowForm] = useState(false); const [title, setTitle] = useState(''); const [clientName, setClientName] = useState(''); const [description, setDescription] = useState(''); const [saving, setSaving] = useState(false);
  const create = async () => {setSaving(true); try {await apiFetch('/enquiries/', {method: 'POST', body: JSON.stringify({title, client_name: clientName, description})}); setTitle(''); setClientName(''); setDescription(''); setShowForm(false); await state.reload();} catch (reason) {Alert.alert('Unable to create enquiry', reason instanceof Error ? reason.message : 'Please try again.');} finally {setSaving(false);}};
...
export function EnquiryDetailScreen({route}: any) {
  ...
  return <ScrollView style={sharedStyles.screen} contentContainerStyle={sharedStyles.content}>
    <Card><View style={sharedStyles.row}><Text style={styles.detailTitle}>{item.title}</Text><StatusPill value={item.status} /></View><Text style={styles.title}>{item.client_name}</Text><Text style={sharedStyles.subtitle}>{item.client_email} {item.client_phone}</Text><Text style={styles.description}>{item.description}</Text></Card>
    {!!item.available_actions?.length && <><Text style={sharedStyles.sectionTitle}>Available actions</Text><Card>
      ...
      {item.available_actions.includes('quote') && <><Field label="Quotation amount" value={amount} onChangeText={setAmount} keyboardType="decimal-pad" /><Field label="Quotation details" value={details} onChangeText={setDetails} multiline /><Button title="Create quotation" onPress={() => void runAction('quote')} loading={saving} disabled={!amount} /></>}
    ...
    <Text style={sharedStyles.sectionTitle}>Quotation versions</Text>{item.quotations?.map(quote => <Card key={quote.id}><View style={sharedStyles.row}><Text style={styles.title}>Version {quote.version} · {quote.amount}</Text><StatusPill value={quote.status} /></View><Text>{quote.details}</Text></Card>)}
